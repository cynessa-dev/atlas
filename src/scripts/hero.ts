...
const getHeroElements = (): HeroElements | null => {
    ...
    const heroContainerElement = document.querySelector<HTMLElement>(
        '[data-hero-container]',
    );
    const heroTaglineElement = document.querySelector<HTMLElement>(
        '[data-hero-tagline]',
    );
    const heroDescriptionElement = document.querySelector<HTMLElement>(
...
const playAnimations = (elements: HeroElements, playIntro: boolean) => {
    ...
    animateHero(
        introTimeline,
        elements.heroContainerElement,
        elements.heroTaglineElement,
        elements.heroDescriptionElement,
        elements.scrambleText,
    );

    if (playIntro) animateZoomOut(introTimeline, elements.propsElement);
    else setPropsToFinalState(elements.propsElement);
